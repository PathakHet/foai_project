...
# When True, every generated successor is stored in GameState.explored. Disable in long experiment runs to keep memory bounded.
TRACK_EXPLORED_STATES = True
...
class ClassicGameRules:
    ...
    def __init__(self, timeout=30, max_moves=None):
...
def readCommand(argv):
    ...
    usageStr = "USAGE: python pacman.py [options]"
    ...
    parser.add_option('-p', '--pacman', dest='pacman',
                      help=default(
                          'agent class to use (KeyboardAgent, LearningReflexAgent, etc.)'),
...
def replayGame(layout, actions, display):
    ...
    agents = [ReflexAgent()] + [ghostAgents.RandomGhost(i+1)
